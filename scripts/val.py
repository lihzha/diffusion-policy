...
def main():
    ...
    cfg.gpu_id = 0
    cfg._target_ = "diffusion.agent.val.val_agent_real.ValAgentReal"
    ...
    predicted_action_unnorm = np.load(
        os.path.join(result_folder, f"predicted_action_unnorm_{args.num_steps}.npy")
    )
    true_action_unnorm = np.load(
        os.path.join(result_folder, f"true_action_unnorm_{args.num_steps}.npy")
    )
    ...
    assert (
        len(predicted_action_unnorm.shape) == 4
        and predicted_action_unnorm.shape == true_action_unnorm.shape
    )
    fig, axs = plt.subplots(4, 2, figsize=(10, 15))
    for i in range(predicted_action_unnorm.shape[-1]):
        error_i = np.abs(
            predicted_action_unnorm[..., i] - true_action_unnorm[..., i]
        ).mean(axis=(1, 2))
        # axs[i//2, i%2].plot(error_i)
        axs[i // 2, i % 2].hist(
            error_i, bins=50, color="skyblue", edgecolor="black", alpha=0.7
        )
