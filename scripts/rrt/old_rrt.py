...
def main(cfg):
    ...
    if record_dir and cfg.render_mode != "human":
        ...
        if cfg.save_video:
            assert cfg.data_rnd<=30, "Cannot save videos when collecting data for more than 30 rounds."
        # Join cfg.env_id, cfg.num_envs, cfg.data_rnd, current date and time to create a unique trajectory name
        trajectory_name = f"{cfg.env_id}_numenvs{cfg.num_envs}_datarnd{cfg.data_rnd}_seed{cfg.seed}" + f"_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    for iteration in range(cfg.data_rnd):
        ...
        goal_cfg_at_global: sapien.Pose = copy.deepcopy(env.get_grasp_pose()) # in global frame
        q0 = env.agent.robot.get_qpos().clone()

   
        goal_cfg_at_base = env.agent.robot.pose.inv() * goal_cfg_at_global
        goal_qpos_for_arm = env.agent.controller.controllers['arm'].kinematics.compute_ik(target_pose=goal_cfg_at_base, q0=q0, use_delta_ik_solver=False)
        ...
        goal_qpos_for_pull_arm = env.agent.controller.controllers['arm'].kinematics.compute_ik(target_pose=goal_pose_for_pull_at_base, q0=q0, use_delta_ik_solver=False)
        goal_qpos_for_pull_gripper = torch.ones_like(goal_qpos_for_pull_arm)[:, :1]*(-1)
        qpos_to_set_pull_start_arm = env.agent.controller.controllers['arm'].kinematics.compute_ik(target_pose=goal_cfg_at_base, q0=q0, use_delta_ik_solver=False)
        ...
        from guided_dc.policy.parallel_rrt import parallel_rrt_connect
        
        def get_extra_cfg():
            return env.agent.tcp.pose.raw_pose.clone()
        
        qpos_paths, ee_global_absolute_paths = parallel_rrt_connect(start_cfg=env.agent.robot.get_qpos(), goal_cfg=qpos_to_set, iterations=300, 
                                        sampler=env.uniform_cfg_sampler, step_size=0.1, collision_checker=env.unwrapped.collision_checker, 
                                        threshold=0.005, get_extra_cfg=get_extra_cfg, start_ee=get_extra_cfg(), goal_ee=goal_cfg_at_global.raw_pose.clone())    # Shape: (num_envs, longest_traj_length, qpos_dim)
        ...
        ee_paths = []
        init_state = env.scene.get_sim_state()
