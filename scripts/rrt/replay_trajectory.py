...
def main(cfg):
    ...
    start_state, actions, control_mode = load_traj_to_replay(cfg.traj_path, cfg.traj_idx)
    env.load_start_state(start_state)
    
    
    assert control_mode == env.unwrapped.control_mode, "Control mode of the trajectory does not match the control mode of the environment"
