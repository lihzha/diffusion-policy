...
def main(cfg):
    ...
    for iteration in range(cfg.data_iter):
        ...
        if iteration >= 1:
            save_extra_info_to_hdf5(
                env,
                {
                    # "success_envs": success_envs.cpu(),
                    "control_mode": env.unwrapped.control_mode,
                    "start_state": env.unwrapped.start_state,
                },
            )
        ...
        qpos_paths, _ = parallel_rrt_connect(
            start_cfg=env.agent.robot.get_qpos(),
            goal_cfg=qpos_to_set,
            iterations=cfg.rrt_iter,
        ...
        qpos_paths = interpolate_trajectory(qpos_paths, 3)
        for _ in range(cfg.smooth_iter):
            qpos_paths = smooth_trajectories(
                qpos_paths, window_size=5, batch_first=True
            )
        ...
        success_envs = env.agent.is_grasping(env.unwrapped.manip_obj)
