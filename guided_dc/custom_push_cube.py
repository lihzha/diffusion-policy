...
from randomization import randomize_continuous
import sapien
...
class CustomPushCubeEnv(BaseEnv):
    ...
    def __init__(self, *args, robot_uids="panda", robot_init_qpos_noise=0.02, **kwargs):
        ...
        self.randomization_config = kwargs.pop('rand_config')
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            self._randomize_obj_pos(b)
            
    def _randomize_lighting(self):
        shadow = self.enable_shadow
        ambient_min = self.randomization_config['lighting']['ambient_min']
        ambient_max = self.randomization_config['lighting']['ambient_max']
    ...
    def _randomize_camera_pose(self):
        rad_range = self.randomization_config['camera']['rad_range']
        pose = sapien_utils.look_at(eye=[0.3, 0, 0.6], target=[-0.1, 0, 0.1])
    ...
    def _randomize_robot_pos(self):
        x_min = self.randomization_config['robot']['pos_range']['x_min']
        x_max = self.randomization_config['robot']['pos_range']['x_max']
        y_min = self.randomization_config['robot']['pos_range']['y_min']
        y_max = self.randomization_config['robot']['pos_range']['y_max']
        x_pos = randomize_continuous(x_min, x_max, 1)
        y_pos = randomize_continuous(y_min, y_max, 1)
        xyz_pos = [x_pos, y_pos, 0]
        return xyz_pos

    def _randomize_obj_pos(self, envs_num):
        ...
        b = envs_num
        pos_scale = self.randomization_config['object']['pos_scale']
        pos_offset = self.randomization_config['object']['pos_offset']
        xyz = torch.zeros((b, 3))
        xyz[..., :2] = torch.rand((b, 2)) * pos_scale + pos_offset
        xyz[..., 2] = self.cube_half_size + 0.1
        q = [1, 0, 0, 0]
        ...
        goal_offset_to_object_min = self.randomization_config['goal']['offset_to_object_min']
        goal_offset_to_object_max = self.randomization_config['goal']['offset_to_object_max']
        goal_offset_to_object = randomize_continuous(goal_offset_to_object_min, goal_offset_to_object_max, 1)
        target_region_xyz = xyz + torch.tensor([goal_offset_to_object + self.goal_radius, 0, 0])
        # set a little bit above 0 so the target is sitting on the table
        target_region_xyz[..., 2] = 0.01
