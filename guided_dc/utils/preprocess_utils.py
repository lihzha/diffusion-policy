...
def batch_apply(x: Batch, func: Callable[[torch.Tensor], torch.Tensor]) -> Batch:
    # Create a list of modified values by applying func to each field
    modified_values = [
        dict_apply(getattr(x, field), func)
        if isinstance(getattr(x, field), dict)
        else func(getattr(x, field))
