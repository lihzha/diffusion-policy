...
def smooth_trajectories(trajectories, window_size=5, batch_first=True):
    ...
    kernel = torch.ones((D, 1, window_size), device=trajectories.device) / window_size  # One filter for each of the 7 dimensions

    # Apply smoothing only to the middle part (excluding the first and last points)
    if batch_first:
        raw_trajectories = trajectories.clone().permute(0, 2, 1)  # Shape: (B, T, D) -> (B, D, T)
    else:
        raw_trajectories = trajectories.clone().permute(1, 2, 0)  # Shape: (T, B, D) -> (B, D, T)
    ...
    if batch_first:
        smoothed_trajectories = smoothed_middle.permute(0, 2, 1)  # (B, D, T) -> (B, T, D)
        smoothed_trajectories[:, 0] = trajectories[:, 0]
        smoothed_trajectories[:, -1] = trajectories[:, -1]
    else:
        smoothed_trajectories = smoothed_middle.permute(2, 0, 1)  # (B, D, T) -> (T, B, D)
        smoothed_trajectories[0] = trajectories[0]
        smoothed_trajectories[-1] = trajectories[-1]
...
def get_waypoints(trajectories, curvature_threshold=0.01, num_waypoints=10):
    ...
    for i in range(B):
        ...
        interp_indices = torch.linspace(0, traj_waypoints.size(0) - 1, num_waypoints, device=trajectories.device)
        interp_waypoints = torch.stack([torch.lerp(traj_waypoints[int(floor)], traj_waypoints[int(ceil)], frac)
                                        for floor, ceil, frac in zip(interp_indices.floor().long(), 
...
def compute_delta_ee_pose_euler(ee_pose):
    ...
    delta_position = positions[:, 1:] - positions[:, :-1]  # Shape (num_envs, traj_length-1, 3)
    
    # Convert quaternions to Euler angles
    euler_angles = quaternion_to_euler_xyz(quaternions)  # Shape (num_envs, traj_length, 3)
    
    # Compute delta euler angles
    delta_euler = euler_angles[:, 1:] - euler_angles[:, :-1]  # Shape (num_envs, traj_length-1, 3)

    # Concatenate delta position and delta euler angles
    delta_ee_pose = torch.cat((delta_position, delta_euler), dim=-1)  # Shape (num_envs, traj_length-1, 6)
...
def interpolate_trajectory(ee_pose_path, k):
    ...
    original_time = torch.linspace(0, 1, traj_len, device=ee_pose_path.device)  # Shape: (traj_len,)
    new_time = torch.linspace(0, 1, new_traj_len, device=ee_pose_path.device)  # Shape: (new_traj_len,)
    ...
    high_idx = torch.clamp(low_idx + 1, max=traj_len - 1)  # Upper bound index, clamped (Shape: (new_traj_len,))
    ...
    low_values = ee_pose_path[:, low_idx, :]  # Shape: (batch_num, new_traj_len, cfg_dim)
    high_values = ee_pose_path[:, high_idx, :]  # Shape: (batch_num, new_traj_len, cfg_dim)
...
def process_paths(ee_paths, goal_poses):
    ...
    for i in range(batch_size):
        ...
        processed_paths[i, nearest_idx+1:] = ee_paths[i, nearest_idx:nearest_idx+1].repeat(traj_len - nearest_idx - 1, 1)
...
if __name__ == '__main__':
    traj_path = '/home/irom-lab/projects/guided-data-collection/videos/PickAndPlace-v1_numenvs4_datarnd1_seedNone_20241010_210747.h5'
    traj_idx = 2
