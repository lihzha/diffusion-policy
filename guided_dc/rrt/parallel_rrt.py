...
class Tree:
    ...
    def find_nearest_node(self, new_node: Node, active_idx: torch.Tensor):
        ...
        nearest_node_cfgs = torch.full((tree_cfgs.size(1), new_cfg.size(1)), PAD_VALUE, device=tree_cfgs.device)
        # nearest_nodes = [None] * tree_cfgs.size(1)
        min_indices_full = torch.tensor([PAD_INDEX]*tree_cfgs.size(1), device=tree_cfgs.device)

        assert active_idx.any()
        # Only perform calculations for active environments
        dist = torch.norm(tree_cfgs[:, active_idx] - new_cfg[active_idx].unsqueeze(0), dim=-1)  # Shape: (num_nodes, num_active_env_idx)
        dist = torch.nan_to_num(dist, nan=torch.inf)
        _, min_indices = torch.min(dist, dim=0)  # Shape: (num_active_env_idx,)
        nearest_node_cfgs[active_idx] = tree_cfgs[min_indices, torch.arange(tree_cfgs.size(1), device=tree_cfgs.device)[active_idx]]
    ...
    def distance(node_1: Node, node_2: Node) -> torch.Tensor:
        ...
        if valid_mask.any():
            dist[valid_mask] = ((cfg_1[valid_mask] - cfg_2[valid_mask]) ** 2).sum(dim=-1).sqrt()
        return dist
    
    def reconstruct_path_with_extra(self, env_done_nodes: dict[int: tuple[Node]], node_order, reverse_path, num_envs, pad_for_failed_envs, pad_for_extra):
        path_dict = {}
        extra_path_dict = {}
    ...
    def reconstruct_path(self, env_done_nodes: dict[int: tuple[Node]], node_order, reverse_path, num_envs, pad_for_failed_envs):
        path_dict = {}
        for env_idx, final_node_pair in env_done_nodes.items():
            path = []
            node = final_node_pair[node_order]
...
def steer(x_nearest: torch.Tensor, x_rand: torch.Tensor, step_size: float, active_env_idx: torch.Tensor):
    # Ensure the input tensors are of the same shape and are torch.Tensor
    assert isinstance(x_nearest, torch.Tensor) and isinstance(x_rand, torch.Tensor), \
        'Inputs must be torch.Tensors for parallel computation.'
    assert x_nearest.shape == x_rand.shape, \
    ...
    if active_env_idx.any():
        direction = x_rand[active_env_idx] - x_nearest[active_env_idx]  # Shape: (num_active_env_idx, cfg_dim)
        norm = torch.norm(direction, dim=-1, keepdim=True)  # Shape: (num_active_env_idx, 1)
        # Avoid division by zero: if norm is zero, we are already at the position, so return x_nearest for those
        norm = torch.where(norm == 0, torch.ones_like(norm), norm)  # Replace zero norms with 1 to avoid division by zero
        # Move from x_nearest towards x_rand, scaled by step_size or up to the distance to x_rand
        step_sizes = torch.min(torch.full_like(norm, step_size, device=direction.device), norm)  # Shape: (num_active_env_idx, 1)
        # Update the new positions for active environments only
        x_new[active_env_idx] = x_nearest[active_env_idx] + direction / norm * step_sizes  # Shape: (num_active_env_idx, cfg_dim)
    return x_new
    
def try_connect(tree_a: Tree, tree_b: Tree, collision_checker, step_size: float, 
                threshold: float, active_env_idx: torch.Tensor, collision_idx: torch.Tensor, 
                env_done_nodes: dict, is_start, get_extra_cfg, use_extra):
    ...
    if active_idx.any():
        ...
        x_nearest_b, nearest_nodes_indices_b = tree_b.find_nearest_node(node_now_a, active_idx=active_idx)
        ...
        new_collision_idx = collision_checker(x_new_a, active_idx)  # Shape: (num_envs, )
        x_new_a[new_collision_idx] = PAD_VALUE
        parent_nodes_indices_a = torch.full((x_now_a.size(0),), PAD_INDEX, device=x_new_a.device)
        parent_nodes_indices_a[active_idx & ~new_collision_idx] = node_now_a_idx
        new_node_a = Node(x_new_a, parent=parent_nodes_indices_a, extra_cfg=get_extra_cfg() if use_extra else None)
        ...
        for i in range(x_now_a.size(0)):
            ...
            if new_active_env_idx[i] == 0 and (new_active_env_idx[i] != active_env_idx[i]):
                new_node_b = Node(x_nearest_b, parent=nearest_nodes_indices_b, extra_cfg=get_extra_cfg() if use_extra else None)
                env_done_nodes[i] = (new_node_a, new_node_b) if not is_start else (new_node_b, new_node_a)

    return env_done_nodes, new_active_env_idx

def parallel_rrt_connect(start_cfg, goal_cfg, iterations: int, sampler, step_size: float, 
                         collision_checker, threshold: float, get_extra_cfg, start_ee, goal_ee, active_joint_indices=[0,1,2,3,4,5,6], pad_inactive_joints_value=1):
    ...
    for iteration in range(iterations):
        ...
        x_rand = sampler(active_env_idx=active_env_idx, pad_value=PAD_VALUE, 
                         active_joint_indices=active_joint_indices, 
                         pad_inactive_joints_value=pad_inactive_joints_value)  # Shape: (num_envs, cfg_dim), PAD_VALUE for done environments
        ...
        x_nearest, nearest_node_indices = tree.find_nearest_node(Node(x_rand), active_idx=active_env_idx)   # Shape: (num_envs, cfg_dim); PAD_INDEX for inactive enviroments
        ...
        x_new = steer(x_nearest, x_rand, step_size, active_env_idx)    # Shape: (num_envs, cfg_dim)
        ...
        tree.add_node(Node(x_new, parent=nearest_node_indices, extra_cfg=get_extra_cfg() if use_extra else None))
        env_done_nodes, active_env_idx = try_connect(tree_a=tree, tree_b=trees[0], 
                                            collision_checker=collision_checker, step_size=step_size, 
                                            threshold=threshold,active_env_idx=active_env_idx, collision_idx=collision_idx,
                                            env_done_nodes=env_done_nodes, is_start=is_start, get_extra_cfg=get_extra_cfg, use_extra=use_extra)

        if len(env_done_nodes) == num_envs:
            break
        print(iteration, active_env_idx)
    if use_extra:
        path_start, extra_path_start = tree_start.reconstruct_path_with_extra(env_done_nodes, node_order=0, reverse_path=True, num_envs=len(active_env_idx), pad_for_failed_envs=start_cfg, pad_for_extra=start_ee)
        path_goal, extra_path_goal = tree_goal.reconstruct_path_with_extra(env_done_nodes, node_order=1, reverse_path=False, num_envs=len(active_env_idx), pad_for_failed_envs=goal_cfg, pad_for_extra=goal_ee)
        return merge_path(path_start, path_goal), merge_path(extra_path_start, extra_path_goal)
    else:
        path_start = tree_start.reconstruct_path(env_done_nodes, node_order=0, reverse_path=True, num_envs=len(active_env_idx), pad_for_failed_envs=start_cfg)
        path_goal = tree_goal.reconstruct_path(env_done_nodes, node_order=1, reverse_path=False, num_envs=len(active_env_idx), pad_for_failed_envs=goal_cfg)
        return merge_path(path_start, path_goal), None
    
def merge_path(path_start: list[list[torch.Tensor]], path_goal: list[list[torch.Tensor]]):
