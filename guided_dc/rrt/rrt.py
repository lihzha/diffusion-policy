...
def steer(x_nearest: Union[np.ndarray, torch.Tensor], x_rand: Union[np.ndarray, torch.Tensor], step_size: float):
    ...
    if isinstance(direction, np.ndarray):
        ...
        if norm == 0:
            return x_nearest  # Already at the position
        # Move from x_nearest towards x_rand, scaled by step_size or up to the distance
        return x_nearest + direction / norm * torch.min(torch.tensor(step_size, device=norm.device), norm)
    
    else:
        raise TypeError("Input must be either numpy.ndarray or torch.Tensor")

def rrt(get_pos, get_goal, sampler, collision_checker, step_size: float, threshold: float, iters=int(1e5)):
    # Initialization
    x_start = get_pos()
    x_goal = get_goal()
    ...
    for _ in range(iters):
        ...
        if not collision_checker(x_nearest, x_new): #TODO: figure out how to simulate paths between x_nearest and x_new
...
def try_connect(tree_a: Tree, tree_b: Tree, collision_checker, step_size: float, threshold: float, ):
...
def rrt_connect(get_pos, get_goal, sampler, collision_checker, step_size: float, threshold: float, iters=int(1e5)):
    # Initialization
    x_start = get_pos()
    x_goal = get_goal()
    ...
    for iteration in range(iters):
        ...
        if not collision_checker(x_nearest, x_new): #TODO: figure out how to simulate paths between x_nearest and x_new
            ...
            node_a, node_b = try_connect(tree, [tree_start, tree_goal].remove(tree), collision_checker, step_size, threshold)
