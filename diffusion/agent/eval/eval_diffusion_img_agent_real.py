...
class EvalImgDiffusionAgentReal(EvalDiffusionAgentReal):
    ...
    def run(self):
        ...
        with torch.no_grad():
            ...
            print("Predicted action chunk dimensions:", samples.trajectories.cpu().numpy().shape)
            naction = samples.trajectories.cpu().numpy()[0,:self.act_steps]  # remove batch dimension
            prev_obs = obs.copy()
        ...
        actions = []
        robot_states = [
            np.concatenate([obs["robot_state"]["cartesian_position"].copy(), 
