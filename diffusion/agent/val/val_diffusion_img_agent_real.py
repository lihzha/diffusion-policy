...
class ValImgDiffusionAgentReal(ValAgentReal):
    ...
    def __init__(self, cfg):
        ...
        self.normalization_stats = np.load(self.normalization_stats_path, allow_pickle=True)
    ...
    def normalize_action(self, action):
        return 2 * (action - self.action_min) / (self.action_max - self.action_min + 1e-6) - 1
    ...
    def run(self):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            predicted_action = self.model.forward(cond=batch.conditions, deterministic=True)[0].cpu().numpy()[:, : self.act_steps]
