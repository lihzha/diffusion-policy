...
class VisionEncoder(nn.Module):
    def __init__(self,
        ...
        if 'vit' in model_name:
            assert self.feature_aggregation in ['spatial_emb', 'compress', 'cls', 'concat', 'mean'], \
                f"feature_aggregation type '{self.feature_aggregation}' not supported"
        elif 'resnet' in model_name:
            assert self.feature_aggregation in ['avgpool', 'compress', 'max'], \
                f"feature_aggregation type '{self.feature_aggregation}' not supported"
        self.get_repr_dim()
    ...
    def get_repr_dim(self):
        x = {f'{i}': torch.randn(1,self.img_cond_steps, *self.img_size) for i in range(self.num_views)}
        ...
        if 'resnet' in self.model_name:  
            ...
            if self.feature_aggregation == 'compress':
                self.raw_repr_dim = self.patch_repr_dim * self.img_cond_steps * self.num_views
            else:
                self.repr_dim = self.patch_repr_dim * self.img_cond_steps * self.num_views
            self.num_patch = None
    ...
    def forward(self, rgb: dict, state=None, aggregate=True):
        ...
        feat = self.model.forward(rgb)  # (bs * img_cond_steps * num_views, patch_nums, embed_dim)
        if aggregate:
    ...
    def aggregate_features(self, x, state=None):
        if 'resnet' in self.model_name:
            ...
            if self.feature_aggregation == 'avgpool':
                ...
                x = einops.rearrange(x, "(b cs v) d-> b (cs v d)", 
                                    cs=self.img_cond_steps, v=self.num_views)
                return x
            x = torch.flatten(x, start_dim=-2) # (batch*img_cond_steps*num_views, emb_dim, h*w)
            x = torch.transpose(x, 1, 2) # (batch*img_cond_steps*num_views, h*w, emb_dim)
            x = einops.rearrange(x, "(b cs v) hw d-> b hw (cs v d)", 
                                    cs=self.img_cond_steps, v=self.num_views)
            if self.feature_aggregation == 'compress':
                x = x.mean(dim=1)
                x = self.nn_compress(x)
        ...
        assert len(x.shape) == 3  # (batch*img_cond_steps*num_views, patch_nums, embed_dim)
        if self.model_name == 'custom_vit':
            if not self.use_large_patch:
                x = einops.rearrange(x, "(v b cs) p d -> b (cs v p) d", cs=self.img_cond_steps, v=self.num_views)
        else:
            x = einops.rearrange(x, "(v b cs) p d -> b (cs v p) d", cs=self.img_cond_steps, v=self.num_views)
        ...
        if self.feature_aggregation == 'spatial_emb':
            ...
            cls_token_idx_list = [self.num_patch * i for i in range(self.num_views * self.img_cond_steps)]
