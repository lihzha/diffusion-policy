...
class TimmEncoder(nn.Module):
    ...
    def forward_loop(self, x: dict):
        ...
        for v, f in zip(x.values(), self.model):
            y = f(v)   # vit: bs*img_cond_steps, patch_nums, embed_dim; resnet: bs*img_cond_steps, embed_dim
            outputs.append(y)
        return torch.cat(outputs, dim=0)  # vit: bs*img_cond_steps*num_views, patch_nums, embed_dim; 
